...
export default class Finger {
	...
	constructor(eventMode, options) {
		...
		this._state = Object.assign({}, {
			...
			// 🖐️option shiftKey: Boolean = false; The value for [`PointerEvent.shiftKey`](https://developer.mozilla.org/docs/Web/API/MouseEvent/shiftKey)
			shiftKey: false
		...
		this.#finalState = Object.assign({}, this._state);
	...
	// 🖐️method shift(): this
	// Sets `shiftKey` for any upcoming states, i.e. emulates pressing the "shift" key.
	// This does *not* trigger a mouse/pointer event by itself.
	shift() {
		this.#finalState.shiftKey = true;
		return this;
	}


	// 🖐️method unshift(options?: {}): this
	// Unsets `shiftKey` for any upcoming states, i.e. emulates depressing the "shift" key.
	// This does *not* trigger a mouse/pointer event by itself.
	unshift() {
		this.#finalState.shiftKey = false;
		return this;
	}
	...
	wait(delay) {
		this._queueMove({finalState: this.#finalState, getState: this._falseFn, duration: delay});
		return this;
	}
	...
	moveTo(x, y, delay=0) {
		return this.moveBy(x - this.#finalState.x, y - this.#finalState.y, delay);
	}
	...
	moveBy(x, y, delay=0) {
		var fromX = this.#finalState.x;
		var fromY = this.#finalState.y;
...
// 			until: this._movesUntil + delay
	...
	_queueMove( move ) {
		...
		this.#finalState = move.finalState = Object.assign({}, this.#finalState, move.finalState);
	...
	// the performance.now() timestamp when the last `*down` event was triggered.
	// #lastDownTimestamp = -Infinity;

	// State to trigger `click` events
...
// 		// TODO: Add jitter if needed
		...
		if (changed) {
			...
			if (previousState.x !== this._state.x || previousState.y !== this._state.y) {
				...
				if (this._state.down &&
					(Math.abs(previousState.x - this._state.x) > 5 ||
					Math.abs(previousState.y - this._state.y) > 5)
				) {
					this.#movedSinceLastDown = true;
				}
			}
			...
			if (previousState.down && (!this._state.down)) {
				...
				if (!this.#movedSinceLastDown) {
					isAlsoClickEvent = true;
				}
				...
				this.#downX = this._state.x;
				this.#downY = this._state.y;
				this.#movedSinceLastDown = false;
			}
		...
		if (this._mode === 'mouse' || this._mode === 'touchpad') {
			...
			/// array.
			/// TODO: Create synthetic and `dblclick` events if/when
			/// needed, add them to the array.
			if (isAlsoClickEvent) {
				return [
					{ type: evType, event: this._asMouseEvent(evType), finger: this },
					{ type: 'click', event: this._asMouseEvent('click'), finger: this }
				];
			} else {
...
// 			target: document.elementFromPoint(this._state.x, this._state.y),	// works with viewport coords
	...
	_asMouseEvent(evType) {
		return new MouseEvent(evType === 'click' ? 'click' : 'mouse' + evType, {
			bubbles: true,
			button:  0,	// Moz doesn't use -1 when no buttons are pressed, WTF?
			buttons: this._state.down ? 1 : 0,
			cancelable: true,
